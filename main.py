...
def get_weather_info(in_weather):
    in_weather_info = in_weather.strip().split()[-1].lower()
...
def get_orbit_info(orbit_str):
    ...
    in_orbit_name, traffic_speed = local[0].lower(), int(local[-2])
    out_orbit_info  = orbit.orbit(in_orbit_name)
    orbit_distance,orbit_crater = out_orbit_info.get_orbit_details()
    return orbit_distance, orbit_crater, traffic_speed


def final_cal(crater_percent, vehicle_list, distance, craters,traffic_speed,result):
    if crater_percent < 0:
    ...
    for vc in vehicle_list:
        ...
        vehicle_speed,crater_time = vc_obj.get_vehicle_details()
...
if __name__ == '__main__':
    ...
    orbit_distance, orbit_crater, traffic_speed = get_orbit_info(in_orb_1)
    result = final_cal(weather_crater_percent, weather_vehicle_list,orbit_distance, orbit_crater, traffic_speed,result_list)
    in_orb_2 = 'Orbit2 traffic speed is 20 megamiles/hour'
    orbit_distance, orbit_crater, traffic_speed = get_orbit_info(in_orb_2)
    print orbit_distance, orbit_crater, traffic_speed
    result = final_cal(weather_crater_percent, weather_vehicle_list,orbit_distance, orbit_crater, traffic_speed, result_list)
    print result
